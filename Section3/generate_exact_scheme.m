function [psi_N] = generate_exact_scheme(num,plot_type,line_type,scheme_type, ...,
    Quadratic_rotation_term)
%FD_SCHEME 此处显示有关此函数的摘要
%   此处显示详细说明
%% 报错
if ismember(scheme_type,{'LCFD'}) == 0
    error('scheme_type 输入错误');
elseif ismember(Quadratic_rotation_term,{'\mathcal{L}_z^{2,I}'}) == 0
...
% 计算第n+1层(n>=1) ========================
for n=1:N-1
    R(j1:jend,k1:kend) = 2./tau.*(2.*psin(j1:jend,k1:kend)-psin_minus(j1:jend,k1:kend)) ...,
    ...
    Ar1=(K_Index+(m-1)).*(number_of_x_points)+J_Index+m;
    Ac1=(K_Index+(m-1)).*(number_of_x_points)+J_Index+m;
    Anum1=ones(number_of_x_points,number_of_y_points);
    Anum1(j1:jend,k1:kend)=tau.*lambda.*abs(psin(j1:jend,k1:kend)).^2;
    A11=sparse(Ar1,Ac1,Anum1,number_of_x_points.*number_of_y_points, ...,
        number_of_x_points.*number_of_y_points);
    A1 = A+A11;

    psin_add = reshape(A1\R_vector,number_of_x_points,number_of_y_points);
    ...
    function [result] = generate_A(coe2)
        ...
        elseif coe2 == 5 % L_zh_2(uh)的系数矩阵
            if strcmp(Quadratic_rotation_term,'\mathcal{L}_z^{2,I}') == 1
                ...
                Auxiliary_metrix=[130,-16,64,-16,-64,-16,64,-16,-64,-64,-8,1,-8,-64,8,-1,8, ...,
