...
% f=sin(X).*sin(Y);fd=reshape(f.',[],1);
% fy=sin(X).*cos(Y);  fyy=-f;
% fyd=reshape(fy.',[],1); fxxd=reshape(fxx.',[],1); fyyd=reshape(fyy.',[],1);
% max(abs(Dy2*fd-fyyd))
psi0=exp(-2*(X.^2+Y.^2)); 
psit0=(1+1i)*psi0; 
...
A=speye((N-1)^2)*(1/k^2+1/2)+.5*(omg^2*xx.^2-speye((N-1)^2))*Dy2+...
    .5*(omg^2*yy.^2-speye((N-1)^2))*Dx2+(omg/k*yy-omg^2/2*xx)*Dx+...
    -(omg/k*xx+omg^2/2*yy)*Dy-omg^2*xx.*yy*Dx*Dy;
B=-speye((N-1)^2)*(1/k^2+1/2)-.5*(omg^2*xx.^2-speye((N-1)^2))*Dy2-...
    .5*(omg^2*yy.^2-speye((N-1)^2))*Dx2+(omg/k*yy+omg^2/2*xx)*Dx+...
    -(omg/k*xx-omg^2/2*yy)*Dy+omg^2*xx.*yy*Dx*Dy;
clear Dx Dy Dx2 Dy2 xx yy x
figure
for j=2:Nmax
    psi2=A\(B*psi0+(2/k^2)*psi1);
    psi0=psi1; psi1=psi2;
%    psimax(j+1)=max(abs(psi1));
   psi=(reshape(psi2,N-1,N-1)).'; 
    pcolor(X,Y,abs(psi)); shading interp
    drawnow
%     j*k
end
% plot(0:k:T,psimax)
